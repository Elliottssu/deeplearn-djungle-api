...
def model_load(language):
    global model,graph
    model = load_model('model/model_' + language + '.h5')
    model._make_predict_function() # tensorflow backend when thread async fixed by djungo
...
def model_predict(s, sentence_max_length=100):
    ...
    with graph.as_default():
        labels = model.predict_classes(s, verbose=0)[0][0]
    return labels
